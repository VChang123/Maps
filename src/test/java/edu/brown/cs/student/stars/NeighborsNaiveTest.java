...
/**
 * Test class for NeighborsNaive.
 */
public class NeighborsNaiveTest extends DistanceCalculator {

  public NeighborsNaive neighbors = new NeighborsNaive();
  ...
  public void testMulti() {
    ...
    assertEquals(neighbors.execute(new ArrayList<>(
        Arrays.asList("neighbors", "-1", "17", "157.683", "-2.3462"))),
        "ERROR: Number of neighbors must be non-negative");

    // neighbors >= number stars

    neighbors.execute(new ArrayList<>(Arrays.asList("neighbors", "7", "0", "0", "0")));
    StarObject actual = neighbors.getNeighborsToPrint().get(6);
    neighbors.execute(new ArrayList<>(Arrays.asList("neighbors", "7", "0", "0", "0")));
    StarObject expected = neighbors.getNeighborsToPrint().get(6);
    assertEquals(actual, expected);

    neighbors.execute(new ArrayList<>(Arrays.asList("neighbors", "7", "0", "0", "0")));
    actual = neighbors.getNeighborsToPrint().get(6);
    neighbors.execute(new ArrayList<>(Arrays.asList("neighbors", "7", "0", "0", "0")));
    expected = neighbors.getNeighborsToPrint().get(6);
    assertEquals(actual, expected);
    ...
    neighbors.execute(new ArrayList<>(Arrays.asList("neighbors", "7", "\"first\"")));
    actual = neighbors.getNeighborsToPrint().get(5);
    neighbors.execute(new ArrayList<>(Arrays.asList("neighbors", "7", "\"first\"")));
    expected = neighbors.getNeighborsToPrint().get(5);
    assertEquals(actual, expected);
  ...
  public void testEmpty() {
    ...
    // star not found
    assertNull(neighbors.execute(new ArrayList<>(Arrays.asList("neighbors", "1", "1.", "27.7191", "1."))));
    assertEquals(neighbors.execute(new ArrayList<>(Arrays.asList("neighbors", "1", "\"second\""))),
        "ERROR: Star not found");
    ...
    // negative neighbors
    assertEquals(neighbors.execute(new ArrayList<>(Arrays.asList("neighbors", "-1", "17",
        "157.683", "-2.3462"))), "ERROR: Number of neighbors must be non-negative");
